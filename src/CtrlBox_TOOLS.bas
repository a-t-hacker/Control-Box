...
#End If
Public Sub hdFormMenuBar(xForm As Object)
    #If VBA7 Then
        Dim xStyle As LongPtr, xFormHndl As LongPtr
    #Else
        Dim xStyle As Long, xFormHndl As Long
    #End If
    xFormHndl = findWindow("ThunderDFrame", xForm.Caption)
    xStyle = GetWindowLong(xFormHndl, GWL_STYLE)
    xStyle = xStyle And Not WS_CAPTION
    SetWindowLong xFormHndl, GWL_STYLE, xStyle
    DrawMenuBar xFormHndl
...
End Sub
Public Sub addOptions(ByVal xType As Byte)
...
If xType = 0 And Range("xlasBlkAddr105").Value2 <> 1 Then CTRLBOX.WindowSel.AddItem ("Maximize               Ctrl+W") Else CTRLBOX.WindowSel.AddItem ("Minimize               Ctrl+W")
...
End Function
Public Function fxsWindow()

hdFormMenuBar CTRLBOX
...
Dim hdTskBar As String
    
hdTskBar = "powershell -command " & """" & "&{$p='HKCU:SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\StuckRects3';$v=(Get-ItemProperty -Path $p).Settings;$v[8]=3;&Set-ItemProperty -Path $p -Name Settings -Value $v;&Stop-Process -f -ProcessName explorer}" & """"

Shell (hdTskBar), vbMinimizedNoFocus

End Sub
Public Sub shTskBar()

Dim shTskBar As String

shTskBar = "powershell -command " & """" & "&{$p='HKCU:SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\StuckRects3';$v=(Get-ItemProperty -Path $p).Settings;$v[8]=2;&Set-ItemProperty -Path $p -Name Settings -Value $v;&Stop-Process -f -ProcessName explorer}" & """"

Shell (shTskBar), vbMinimizedNoFocus
